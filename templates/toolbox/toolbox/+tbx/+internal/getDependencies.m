function Info = getDependencies(target, varargin)
   ...
   % NOTE: Need to update this for +tbx so it defaults to generate a
   % requirements file for the project.

   % parse inputs
   [fileList, referenceList, saveRequirementsFile] = parseinputs( ...
      target, mfilename, varargin{:});
   ...
   if saveRequirementsFile
      save(requirementsFile, 'requirementsList', 'urlList')
   end
end
...
%% input parsing
function [targetList, referenceList, saveRequirementsFile] = parseinputs( ...
      target, mfuncname, varargin)
   ...
   parser.addParameter('saveRequirementsFile', '', @ischarlike);
   parser.parse(target, varargin{:});

   referencePath = parser.Results.referencePath;
   saveRequirementsFile = parser.Results.referencePath;
