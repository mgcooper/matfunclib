%TEST_ISSCALARTEXT test isscalartext
...
assert(isnan(isscalartext(NaN)));
assert(isinf(isscalartext(Inf)));
assert(abs(isscalartext(1e200) - 1e200) < 1e-10); % replace with theoretical result
...
eid = 'matfunclib:libname:isscalartext';

% verify failure
assertError(@() isscalartext('inputs that error'), eid, testdiag)

% verify success
assertSuccess(@() isscalartext('inputs that dont error'), eid, testdiag)
...
assert(isequal(isscalartext(X), expected));

expected = []; % add expected value for single type
assert(isequal(isscalartext(single(X)), single(expected)));

expected = []; % add expected value for logical type
assert(isequal(isscalartext(logical(X)), expected));

expected = []; % add expected value for int16 type
assert(isequal(isscalartext(int16(X)), expected)); % int16

%% Test dimension handling

% Test different dimensions
assert(isequal(isscalartext([2 3]), [4 6])); % 1D array
assert(isequal(isscalartext([2 3; 4 5]), [4 6; 8 10])); % 2D array
