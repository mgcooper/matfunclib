...
% or valid referencing vector/matrix R as a 2-d labeled contour plot using
...
validateattributes(Z, ...
   {'numeric', 'logical'}, ...
   {'size', R.RasterSize}, mfilename, 'Z', 1)
...
if strcmp(R.CoordinateSystemType,'planar')
   h = mapshow(Z,R,'DisplayType','contour', 'ShowText','on', varargin{:});
   
elseif strcmp(R.CoordinateSystemType,'geographic')
   h = geoshow(Z,R,'DisplayType','contour', 'ShowText','on', varargin{:});
