...
%RASTERIZE constructs a spatially referenced raster Z and
...
%   [Z,R] = RASTERIZE(x,y,z,rasterSize) constructs
...
%   [Z,R] = RASTERIZE(x,y,z,cellextentX,cellextentY) allows the
...
%   querying multiple values in repeated calls, but is not faster than
%   griddata when computing a single surface as in this function. Moreover
%   griddata includes the interpolation methods 'cubic' and 'v4'
%   (biharmonic spline) in addition to the 'linear', 'nearest', and
%   'natural' methods of scatteredInterpolant. One limitation of griddata
%   is that extrapolation is not supported, but in principle extrapolation
%   should not be used for geophysical raster data. A future release might
%   update to file_exchange function gridfit. More work is needed to
%   determine if that function is a better choice.
...
%     yoffset     =   (max(y(:))-min(y(:)))/10;
  ...
  % exclude grid cells outside the range of values ... 
