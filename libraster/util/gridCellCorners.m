function [XV, YV] = gridCellCorners(X, Y, cellSizeX, cellSizeY, Orientation)
   %GRIDCELLCORNERS Compute corner vertex coordinates of regular grid.
   %
   % [XV, YV] = gridCellCorners(X, Y, cellSizeX, cellSizeY, Orientation)
   % 
   % See also: gridNodesToEdges
   ...
   switch Orientation
      ...
      case 'ccw'
         ...
         % Test if adding an additional vertex changes the behavior of
         % subsequent Matlab computational geometry methods (appears it doesn't
         % - if 'ccw' is used here, polyshape(XV,YV) will produce a 'cw'
         % orientation, so the default orientation used here is 'cw'.
