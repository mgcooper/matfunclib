...
% coordinate arrays ("fullgrids") Xedges and Yedges, given the input X
% and Y grid vectors or coordinate arrays.
%
% [Xedges, Yedges] = gridNodesToEdges(X, Y, gridOption) returns the cell edges
% as coordinate arrays if gridOption is "fullgrids", or as grid vectors if
% gridOption is "gridvectors".
%
% Input:
...
elseif gridType == "point"
   if nargin < 4
      error('The input X,Y data are scalar. This function requires cellsizex and cellsizey inputs.');
   else
...
% For reference, before using gridvec, both were sorted in ascending order
