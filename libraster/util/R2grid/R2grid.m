function [X,Y] = R2grid(R)
   ...
   %   \   = R.XWorldLimits(1)                            = R.XWorldLimits(2)
   %    \  = R.XIntrinsicLimits(1) = 0.5                  = R.XIntrinsicLimits(2)
   %     \
   %      origin
   % __________________
   % VERTICAL DIMENSION
   ...
   %
   % This is why when starting with an R object, creating a grid involves
   % adjusting the X/YWorldLimits (or Latitude/LongitudeLimits) INWARD by 1/2
   % cell size.
   %
   % If instead, the raster cell centers are used to construct an R object, the
   % opposite is true - the min/max cell centers are adjusted OUTWARD by 1/2
   % cell size to create the xlimits/ylimits inputs to MAPREFCELLS or
   % GEOREFCELLS.
