function [gridType, cellSizeX, cellSizeY, tfGeoCoords] = mapGridInfo(X, Y, gridFormat)
...
% both will be matrices with the same number of elements. See mapGridCellSize
% which attempts to do so. Also, this step may be unneccesary, or should be done
% in a dedicated function, called by mapGridCellSize. The conversion of X, Y is
% more complete in prepareMapGrid, but there the convesion is not necessarily to
% gridvectors.
switch gridFormat 
   case {'fullgrids', 'gridvectors'}
   ...
   % Remove duplicate coordinate pairs. Use 'stable' if order needs to be kept.
   XY = unique([X(:),Y(:)], 'rows');
   X = XY(:, 1);
   Y = XY(:, 2);
