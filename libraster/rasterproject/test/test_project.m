% THis is an old matlab function called 'project.m' that was removed in
% R2013b. I found it by looking in the release notes. It doesn't say what
% replaced it. In R2013a, there were functions 'comet' and 'comet3' that
% were replaced by mfwdtran: cometm(lat,lon,p) is now [x,y] =
% mfwdtran(lat,lon) and cometm3 is now [x,y,z] = mfwdtran(lat,lon,z). Note
% that mfwdtan now uses syntax [x,y,alt] where alt is altitutde. 

% main takeawy is jsut that I copied this here because they might remove
% the code altogethr so I wanted a cpy

function test_project(h,option)
            ...
            case 'patch'
                ...
                if length(lat) == 1		% point data; undo closure of patch
                    ...
                    elseif isempty(FVCD) || length(FVCD) == 1 % standard mapping toolbox patches
                        ...
                        [x,y,~,savepts] = mfwdtran(lat,lon,z,'none');
