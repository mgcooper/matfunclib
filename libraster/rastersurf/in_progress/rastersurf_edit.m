...
% ToDO: see error handling, trying to get it to convert refvec and/or
% refmat into R but confused on try catch 

% might also get lat/lon limits and initialize a worldmap for the region,
% since the default behavior seems to be crappy for polar regions
...
%R as a 2-d surface using in-built Matlab functions mapshow.m or geoshow.m
%(Copyright 2016 The MathWorks, Inc.). Default behavior sets properties
%('DisplayType','surface'), ('Box','on'), ('TickDir','out'),
...
%             catch
%                 disp(['Expected input argument 2, R, to be a referencing ' ...
...
if (~isa(R,'map.rasterref.MapCellsReference')) && ...
    ...
    try 
        ...
        rethrow(ME) % don't try anything fancy, just quit
    end
end
...
%     catch ME
%         switch ME.message
%             case 'Expected input number 1, REFVEC, to be a vector.'
%             % case % 
%         
% 
%         disp(['Expected input argument 2, R, to be a referencing ' ...
...
% The two statements below can be used to require
% Map/GeographicCellsReference object as input instead of the solution
% above that attempts to convert refvec/refmat into
% Map/GeographicCellsReference objects

% % confirm R is either a MapCells or GeographicCellsReference objects
...
% % confirm R is either a MapCells or GeographicCellsReference objects
% assert( (isa(R,'map.rasterref.MapCellsReference')) || ...
%         (isa(R,'map.rasterref.GeographicCellsReference')) , ...
%         ['Expected input argurment 2, R, to be of class ' ...
%         '''map.rasterref.MapCellsReference'' or ' ...
%         '''map.rasterref.GeographicCellsReference. Instead it was of class ' ...
%         class(R) '. If working with geographic data, try ' ...
%         '''refmatToGeoRasterReference'' or ''refvecToGeoRasterReference''.' ...
%         ' If working with planar (projected) data, try ' ...
