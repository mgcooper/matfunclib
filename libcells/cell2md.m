function varargout = cell2md(X, varargin)
   ...
   % __________________________________________________________________________
   %  EXAMPLE USAGE
   %    X = num2cell(rand(10, 3)); Save with default filename and alignment with
   %    column headers A, B, C cell2md(X, 'hdr', {'A' 'B' 'C'});
   %
   %
   % ---------------------- Copyright (C) 2015 Bob Spunt ----------------------
   % 	Created:  2015-06-11 Email:    spunt@caltech.edu
   % __________________________________________________________________________
   ...
   if ~iscell(X)
      fprintf( ...
         ['| WARNING: Input is not a cell array and will be converted to cell. ' ...
      ...
      if any(notchar(:))
         fprintf( ...
            ['| WARNING: Some cells do not contain CHAR data and will be ' ...
            'converted to CHAR. Check output carefully.\n']);
         try
   ...
   elseif length(hdrnames)~=size(X, 2)
      error( ...
         'Length of colnames (%d) does not equal number of columns in X (%d)', ...
         length(hdrnames), size(X, 2));
   end

   % | Check ALIGN
   if ischar(alignment)
      alignment = cellstr(alignment);
   end
   ...
   elseif length(alignment)~=size(X, 2)
      error( ...
         'Length of ALIGNMENT (%d) does not equal number of columns in X (%d)', ...
         length(alignment), size(X, 2));
   end
...
function argstruct = setargs(defaults, optargs)
   ...
   % "name, default value" pairs for all optional arguments. The second argument
   % is optional and should be a cell array of "name, custom value" pairs for at
   ...
   if ~isempty(optargs)
      if mod(length(optargs), 2)
         error( ...
            ['Optional inputs must be entered as Name, Value pairs, ' ...
            'e.g., myfunction(''name'', value)']);
      end
      ...
      for i = 1:size(arg,1)
         ...
         if sum(idx) > 1
            error( ...
               ['Input "%s" matches multiple valid inputs:' ...
               repmat('  %s', 1, sum(idx))], ...
               arg{i,1}, defaults{idx, 1});
         elseif ~any(idx)
   ...
   for i = 1:size(defaults,1)
      assignin('caller', defaults{i,1}, defaults{i,2});
   end
   if nargout>0
      argstruct = cell2struct(defaults(:,2), defaults(:,1));
   end
end
...
% ---------------------- Copyright (C) 2015 Bob Spunt ----------------------
%	Created:  2015-03-11 Email:    spunt@caltech.edu
%
