function [result, why] = isequalcell(cell1, cell2, funh2string, ignorenan)
   %ISEQUALCELL performs an equality comparison between two cell arrays
   %
   % ISEQUALCELL performs an equality comparison between two cell arrays by
   % recursively comparing the elements of the cell array, their values and
   % sub-values
   %
   % USAGE:
   %
   % isequalcell(cell1, cell2)
   ...
   %
   % [iseq, info] = isequalcell(cell1, cell2)
   ...
   %
   % [...] = isequalcell(cell1, cell2, funh2string, ignorenan)
   ...
   % isequalcell(c1, c2)
   % [iseq, info] = isequalcell(c1, c2)
   % [iseq, info] = isequalcell(c1, c2, true)
   % [iseq, info] = isequalcell(c1, c2, true, true)
   if nargin < 3
      funh2string = false;
   end
   if nargin < 4
