function [result, why] = isequalstruct(struct1, struct2, funh2string, ignorenan)
   ...
   %
   % [iseq, info] = isequalstruct(struct1, struct2)
   ...
   %
   % isequalstruct(hg1, hg2)
   % [iseq, info] = isequalstruct(hg1, hg2)
   % [iseq, info] = isequalstruct(hg1, hg2, true)
   if nargin < 3
      funh2string = false;
   end
   if nargin < 4
