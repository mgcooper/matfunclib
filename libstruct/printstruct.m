function varargout = printstruct(S, varargin)
   ...
   for i = 1:length(S)
      ...
      if length(cS)==1
         str = recFieldPrint(cS, nindent, nlevels, printcontents, ...
            maxarraylength, structname, sortfields);
         str = [cellstr(structname); str];
      else
         str = [];
         for i = 1:length(cS)
            tmpstr = recFieldPrint(cS(i), nindent, nlevels, printcontents, ...
               maxarraylength, structname, sortfields);
...
function argstruct = setargs(defaults, optargs)
   ...
   if ~isempty(optargs)
      if mod(length(optargs), 2)
         error( ...
            ['Optional inputs must be entered as Name, Value pairs, e.g., ' ...
