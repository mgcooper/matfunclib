function S = mergestructs(S, T)
   %MERGESTRUCTS Combine fields of input structures into a single structure
   %
   %   Output structure will contain all fields and their corresponding values
   %   of the two input structures. If a field is present in both input
   %   structures, the field in S is used, and a warning is produced. This
   %   warning can be turned off using warning('off', "mergestructs:commonfield")
   %
   %   Example:
   %   S.one = 1;
   %   T.two = 2;
