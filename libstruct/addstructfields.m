function S = addstructfields(S, newfields, varargin)
   %ADDSTRUCTFIELDS Add new fields to struct.
   %
   % Syntax
   %
   %  S = ADDSTRUCTFIELDS(S, NEWFIELDS) adds NEWFIELDS to struct S using
   %  inputname(NEWFIELDS) to name the new fields. S = ADDSTRUCTFIELDS(S,
   %  NEWFIELDS, 'NEWFIELDNAMES', NAMES) uses NAMES to name the new fields.
   %
   ...
   % TODO rename assignNonScalarStructFields, might also be worth abandoning
   % this, its akward, I think it is designed to assign to non-scalar structs,
   % which is tricky, so
...
% could go about it like this: switch class(field)
%
%    case 'cell'
%       [S.(fieldname)] = field{:};
%
