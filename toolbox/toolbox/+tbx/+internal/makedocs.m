function makedocs(varargin)
   ...
   %
   %
   % Note: If the repo is configured to publish from docs/ the default action
   ...
   % TODO: accept docslist or generate it from expected location, and check for
   % alternate spelling/location or prescribed gettingStarted file

   if opts.docpages == true

      % publish the standard docs using matlab's publish
      for n = 1:numel(docslist)
         publish(fullfile(docspath, docslist{n}), mpubopts);
      end

      % for github actions the landing page must be saved as docs/index.html
      % where docs/ is at the top-level.
      if isfile(fullfile(htmlpath, 'GettingStarted.html'))
         copyfile( ...
            fullfile(htmlpath, 'GettingStarted.html'), ...
