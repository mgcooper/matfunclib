function updatecontents(folder)
   ...
   % write to output. Lines composed from padded name columns pass
   % through deblank, and the separator and timestamp lines are
   % written without trailing whitespace, so the generated file
   % carries none. The editor's smart indent strips trailing
   % whitespace, so a generator that writes it would reintroduce a
   % diff on every regeneration.
   nrows = length(files);
   fid = fopen(filename, 'w'); % open file for writing
   fprintf(fid, '%s\n%%\n', deblank(['% ' upper(name)]));
   fprintf(fid, '%s\n', deblank(['%   Contents file for ' upper(folder(fIX+1:end)) ' and its subfolders.']));
   for row=1:nrows
      if isempty(H1_lines{row})
         ...
         fprintf(fid, '%s\n', deblank(['%   ' rowfilename repmat(' ',1,longest_word-length(rowfilename)) ' - ' H1_lines{row,:}]));
      end
   end
