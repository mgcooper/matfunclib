function varargout = plotting_tricks(varargin)
   ...
   % FOr a good example of eastings and northings with 10^5, see map_sw_albedo
   % in runoff project (likely will be renamed but the scrip tthat creates the
   % albedo comparison at sw sector scale bw modis,mar,merra,and racmo), it
   % removed the exponent but the trick is saving the ticklabels first:
   ylabel('Northing [m x 10^6]')
   yTickLabels = ax.YAxis.TickLabels; % only saves the base i.e. not the ^10
   ax.YAxis.Exponent = 0;
