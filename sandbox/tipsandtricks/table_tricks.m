function varargout = table_tricks(varargin)
   %TABLE_TRICKS table tips and tricks
   ...
   %%
   % DONT USE dotAssign!! or dotIndex!! Use brace indexing and assigning, dot
   % applies to every single row individually, basically its not vectorized.

   % The syntax T.(varName)() calls tabular.dotReference for every single row
   % and is extremely slow. Similarly the second line calls tabular.dotAssign
   % for every single row. The brace indexing above is
   % offset = T.(varName)(start - 1, :) - T.(varName)(start, :);
   % T.(varName)(start:stop, :) = T.(varName)(start:stop, :) + offset;
   %%
