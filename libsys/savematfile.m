function msg = savematfile(filename,variables,varargin)
   %SAVEMATFILE Save workspace variables to matfile.
   %
   %  msg = SAVEMATFILE(filename,variables) is identical built-in save function
   %
   %  msg = SAVEMATFILE(filename,variables,savefile) is identical to built-in
   %  save function if savefile is TRUE. If savefile is FALSE, nothing is done.
   %  This option removes the need to include if-else statements in code.
   %
