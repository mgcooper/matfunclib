function varargout = pathadd(pathlist, addSubPaths, pathAppend, ignorePaths, opts)
   ...
   %
   % pathAppend: char or string, '-begin' adds specified folders to the top of
   % the search path, '-end' adds them to the end. Default is '-end'.
   %
   ...
   % character (package folders), folders named resources, or subfolders within
   % any of these. Therefore these will be ignored by default. In addition,
   % '.git', '.svn,', 'CVS', '.' and '..' are ignored.
   %
   ...
   ignorePaths = [ignorePaths; ".git"; ".svn"; "CVS"; "."; ".."; "resources"];
   ...
   for n = 1:numel(pathlist)
      ...
      if addSubPaths
         ...
         % Rebuild the path string. Insert platform-specific path separator
         % between each path in the subpaths cell array, then join them
         % horizontally.
