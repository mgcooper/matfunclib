...
%GETFOLDERLIST Returns a list of folders under a specified top-level directory.
%
%  FLIST = GETFOLDERLIST(TOPPATH) Returns a cell array of folder names under the
%  top-level directory specified by TOPPATH. It does not include subdirectories.
%
...
%  TOPPATH. NUMLEVELS=1 returns those folders plus their subfolders, and so on.
%
%  FLIST = GETFOLDERLIST(TOPPATH, NUMLEVELS, OPTION) Specifies the format of
%  the output list. Options: 'foldernames' (default) returns only the folder names,
%  'relativepaths' returns the relative path from TOPPATH, 'fullpaths' returns the
