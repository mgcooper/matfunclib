% In this version, the default value assigned to varnames is string.empty(),
% which means we have to assign the defaults and validate them.
...
function T = dropcats(T, varnames)
   ...
   arguments
      ...
      varnames (1, :) string = string.empty() % Optional variable names
   end
   
   % If varnames is not provided, find all categorical variables
   if isempty(varnames)
      varnames = string(T(:, vartype('categorical')).Properties.VariableNames);
   else
