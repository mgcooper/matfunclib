function deblankEquals(varargin)
   ...
   for n = 1:numel(Lines)
      ...
      if ~isempty(strfind(Lines{n},'='))
         ...
         % Tried to do ... continuataions but not worth the hassle since
         % sometimes they start lines
      end
   end
   ...
   % Re-select the text
   [sel(3), sel(4)] = matlab.desktop.editor.indexToPositionInLine( ...
      aed, numel([PreText newLines(1:end-1)]));
   aed.Selection = sel;
end
