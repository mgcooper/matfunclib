function s = char2cell(s,delim,rowseparate,trim)
   %CHAR2CELL Cast char to cell str array.
   %
   % Syntax:
   ...
   %       If a cell array then each cell element is a delimeter or
   %       multi-element delimeter.
   %       The following may be used for non-printing characters: (as these
   %       characters require 2 elements, they must be specified in a cell array)
   %           \b - backspace
   %           \f - formfeed
   %           \n - linefeed
   %           \r - carriage return
   ...
   %       depending upon "rowseparate". If "empty", then all matrices are
   %       treated as 1D. e.g. a KxMxN matrix is treated as a Kx(MxN) matrix if
   %       "false" or as a Mx(KxN) if "true".
