function stringthing = strrepl(stringthing, oldstr, newstr)
   ...
   % oldstr in a string-like thing with newstr. A string-like thing is a 1xN
   % char array, cellstr array, or string array. Stringthing is returned in the
   % same format as it was passed in as, but is converted to string to perform
   % the processing.
   %
   % Note: this is *replaced* by the built in function 'replace'. It tries to
   % use that first, and if that fails, uses a backup method.
   %
   % Example:
