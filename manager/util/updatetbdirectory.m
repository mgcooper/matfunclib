...
% checks if the source path exists for each toolbox at the top-level or within
% sub-libraries. If the source path is found, it is updated in the directory.
% If not, the corresponding pathexists field is set to false.
%
...
assert(strcmp(option, 'paths'), "Invalid option. The only available option is 'paths'.");
...
for tbname = allnames
   ...
   if isfolder(tbpath)
      ...
      if isfolder(fullfile(gettbsourcepath(), tbname))
         ...
         sublibs = getfolderlist(gettbsourcepath(), 1, 'relativepaths');
