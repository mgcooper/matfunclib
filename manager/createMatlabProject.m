...
%
% proj = createMatlabProject(projectFolder, addProjectFiles, addChildFiles, projectSubfolders, projectName)
%
% Inputs
%
...
if addChildFiles
   cellfun(@(folder) proj.addFolderIncludingChildFiles(folder), projectSubfolders);
else
   % Add only files in the top-level of each projectSubfolder
   cellfun(@(folder) proj.addFile(folder), projectSubfolders);
end
...
if ismissing(projectSubfolders)
   ...
   projectSubfolders = allSubfolders(ismember({allSubfolders.name},projectSubfolders));
end
...
% Remove .git, .svn, and resources folders.
projectSubfolders = projectSubfolders(~contains({projectSubfolders.folder},ignore));
projectSubfolders = projectSubfolders(~ismember({projectSubfolders.name},ignore));

% Convert from dir struct to full path.
projectSubfolders = fullfile({projectSubfolders.folder}',{projectSubfolders.name}');

% Get a list of all files in the project folder.
projectFiles = listallfiles(projectSubfolders);
