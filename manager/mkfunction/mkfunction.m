function mkfunction(funcname,varargin)
   ...
   % isfile should be sufficient to catch the case where parseFunctionPath
   % fences off the new function in a parent folder with the same name as the
   % function to avoid over-riding functionSignatures
...
function mknewfunc(functionpath,filenamepath,funcname,parent,parser)
   ...
   % replace YYYY in the license
   try
      license = strrep(license,'Copyright (c) YYYY,', ...
