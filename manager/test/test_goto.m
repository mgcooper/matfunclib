...
assert(isnan(goto(NaN)));
assert(isinf(goto(Inf)));
assert(abs(goto(1e200) - 1e200) < 1e-10); % replace with theoretical result
...
eid = 'matfunclib:libname:goto';

% verify failure
assertError(@() goto('inputs that error'), eid, testdiag)

% verify success
assertSuccess(@() goto('inputs that dont error'), eid, testdiag)
...
assert(isequal(goto(X), expected));

expected = []; % add expected value for single type
assert(isequal(goto(single(X)), single(expected)));

expected = []; % add expected value for logical type
assert(isequal(goto(logical(X)), expected));

expected = []; % add expected value for int16 type
assert(isequal(goto(int16(X)), expected)); % int16

%% Test dimension handling

% Test different dimensions
assert(isequal(goto([2 3]), [4 6])); % 1D array
assert(isequal(goto([2 3; 4 5]), [4 6; 8 10])); % 2D array
