...
%SETARGS parse name/value arguments in varargin and assign default values
%
%  argstruct = setargs(defaultargs, varargs)
%
...
%
% 	DEFAULTARGS
%     cell array of default "Name', value" pairs for all variables
%
% 	VARARGS [optional]
...
if ~isempty(varargs)
   ...
   for n = 1:size(arg,1)
      idx = strncmpi(defaultargs(:,1), arg{n,1}, length(arg{n,1}));
      if sum(idx) > 1
         error(['Input "%s" matches multiple valid inputs:' ...
            repmat('  %s', 1, sum(idx))], arg{n,1}, defaultargs{idx, 1});
      elseif ~any(idx)
         error('Input "%s" does not match a valid input.', arg{n,1});
      else
         defaultargs{idx,2} = arg{n,2};
      end
   end
end

for n = 1:size(defaultargs,1)
   assignin('caller', defaultargs{n,1}, defaultargs{n,2}); 
end

if nargout>0
   argstruct = cell2struct(defaultargs(:,2), defaultargs(:,1)); 
end
