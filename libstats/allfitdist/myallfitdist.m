...
p.addParameter(   'sortby',         'BIC',            @(x)ischar(x)        );
p.addParameter(   'normalization',  'PDF',            @(x)ischar(x)        );
p.addParameter(   'frequencies',    nan,              @(x)isnumeric(x)     );
p.addParameter(   'isdescrete',     false,            @(x)islogical(x)     );


p.parseMagically('caller');

% mgc TODO: add remaining parameter options in the signature file
% finsih translating the input parsing 
% I commented out the inptu parsing below that should be taken care of, it
% got complicated on 'vin'
...
%% Plot if requested
if plotind
    h = plotfigs(data,D,PD,vin,strs,plotpdf,discind);
end
...
function h = plotfigs(data,D,PD,vin,strs,plotpdf,discind)
...
if plotpdf
    if ~discind
        %Continuous Data
        % mgc: this means nbins is always 50 or 100
        ...
        ys = cat(2,ys{:});
        h.b = bar(xi,fi,'FaceColor',[160 188 254]/255,'EdgeColor','k');
        hold on;
        h.p1 = plot(xi2,ys,'LineWidth',1.5);
...
%Cumulative Distribution
    if ~discind
        ...
        h.s1 = subplot(2,1,1);
        if lgx
            h.p2 = semilogx(xi,fi,'k',xi,ys);
        else
            h.p2 = plot(xi,fi,'k',xi,ys);
        end
        legend(['empirical',{D(inds).DistName}],'Location','NE')
        xlabel('Value');
        ...
        if lgx
            h.p3 = semilogx(xi,bsxfun(@minus,ys,fi));
        else
            h.p3 = plot(xi,bsxfun(@minus,ys,fi));
        end
        ybnds = max(abs(y(:)));
        ax = axis;
        ...
        h.s2 = subplot(2,1,2);
        y = 1.1*bsxfun(@minus,ys,fi);
        h.p2 = stairs(xi,bsxfun(@minus,ys,fi));
        ybnds = max(abs(y(:)));
        ax = axis;
        axis([ax(1:2) -ybnds ybnds]);
        legend({D(inds).DistName},'Location','NE')
...
function distname = defaultdistnames
