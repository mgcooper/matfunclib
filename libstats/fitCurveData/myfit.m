...
validx = @(x)validateattributes(x,{'numeric'},{'real','vector'},'myfit','x',1);
validy = @(x)validateattributes(x,{'numeric'},{'real','vector'},'myfit','y',2);
...
for n = 1:numel(fields)
   ...
   myvarargin{2*n} = p.Unmatched.(fields{n});
end
...
linearLibrary = {'linear','quadratic','abline','originline','trig', ...
   'trigoffset','plane','general2nd','right'};
