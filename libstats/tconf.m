...
%TCONF returns 1-alpha confidence intervals given mean mu, standard
%deviation sig, sample size N, and confidence level alpha. No adjustment is
%made to N, so pass in N for a sample statistic such as the mean of a
%sample, and N-1 if mu/sig are the mean/stdv of a sample of regression
%coefficients (2-parameter model). alpha=.32 means 68% CI, alpha = 0.05
%means 95% CI. Default is 0.32, which correspond to an estimate of the
%population standard deviation, the standard uncertainty.
% 
% Output:
