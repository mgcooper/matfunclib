function [ nse ] = nashsutcliffe(obs, mod, opt)
   ...
   %
   %  [ nse ] = nashsutcliffe(obs, mod) computes a modified form of the NSE
   %  metric where the reference value in the denominator is the model mean
   ...
   assert(numel(obs)==numel(mod), ...
      'OBS and MOD must have the same number of elements');
