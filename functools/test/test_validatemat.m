%TEST_VALIDATEMAT test validatemat
...
assert(isnan(validatemat(NaN)));
assert(isinf(validatemat(Inf)));
assert(abs(validatemat(1e200) - 1e200) < 1e-10); % replace with theoretical result
...
eid = 'matfunclib:libname:validatemat';

% verify failure
assertError(@() validatemat('inputs that error'), eid, testdiag)

% verify success
assertSuccess(@() validatemat('inputs that dont error'), eid, testdiag)
...
assert(isequal(validatemat(X), expected));

expected = []; % add expected value for single type
assert(isequal(validatemat(single(X)), single(expected)));

expected = []; % add expected value for logical type
assert(isequal(validatemat(logical(X)), expected));

expected = []; % add expected value for int16 type
assert(isequal(validatemat(int16(X)), expected)); % int16

%% Test dimension handling

% Test different dimensions
assert(isequal(validatemat([2 3]), [4 6])); % 1D array
assert(isequal(validatemat([2 3; 4 5]), [4 6; 8 10])); % 2D array
