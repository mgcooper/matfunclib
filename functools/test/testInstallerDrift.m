classdef testInstallerDrift < matlab.unittest.TestCase
   ...
   properties (Constant)
      ...
      CarveOut = { ...
         ...
         [ ...
         "      % The bootstrapped copy derives this default in parseargs: the"; ...
         "      % toolbox ""dependencies"" folder, not projectPath/dependencies."; ...
         "      kwargs.installPath (1, :) string {mustBeTextScalar} ..."; ...
         "         = """" % """" defers to parseargs"]; ...
         [ ...
   ...
   methods (Test)
      ...
      function testWithcdCopyIsByteIdentical(testCase)
         % The template installer calls withcd, and private-function
         % precedence resolves that call to the copy stamped beside it, not
         % to libsys. The copy must match libsys/withcd.m byte for byte. A
         % difference gives packaged toolboxes a different withcd, and no
         % other test in this suite detects it.
         expected = fileread(fullfile(testCase.repoRoot, "libsys", ...
            "withcd.m"));
         returned = fileread(fullfile(testCase.repoRoot, "toolbox", ...
            "toolbox", "+tbx", "+internal", "private", "withcd.m"));
         testCase.verifyEqual(returned, expected)
      end
