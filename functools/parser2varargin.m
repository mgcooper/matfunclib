...
% but espectially optional and/or flags. THIS SHOULD BE REPLACED by an
% additioanl method in magicParser that adds the required args to the parser
% object, then i could just use p.UsingDefaults and the known required args to
% dtermine which inputs are NOT using defaults and pass those to shaperead or
% whatever built-in I am wrapping around.

% this was very confusing, but basically, this should be renamed something like:
% defaults2varargin, unless there really are additional use cases that justify a
% general function parser2varargin. the problem this solves is the following: I
% want to write a function that wraps around a matlab built in function and i
% want to be able to pass the default syntax for that built-in function into my
% wrapper function. But I want to pass the built-in syntax as varargin{:}
% because the built-in function doesn't use inputParser and/or it has inputs
% that don't have default values, so i cannot mirror those default values in my
% input parser and I don't know which default values would work, in general. If
% I did know, then I could just use a full call to the built-in. For example,
% this came up in shaperead. If I knew good default values, my loadgis function
% could just call shaperead like: 
% S = shaperead(fname,'BoundingBox',defaultbbox,'Attributes',defaultatts, ...) 
% and so on. But without good default values, i cannot do this, and instead, I need
% to have a switch block or a bunch of if/else statements that determine which
% values were passed into my function and then execute each calling syntax of
% shaperead. To avoid that, i want to use input parser to determine which
...
switch option
   ...
   % here I was gonna move unmatched2varargin here, in case theres an edge case
   % that isn't satisfied by namedargs2cell, so I have all the various
