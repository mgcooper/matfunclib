function [ flags,pct_badQA,flag_types ] = metQA( data, data_type, min_lim, ...
                            max_lim, roc_lim, noc_ignore, no_data )
...
mini            =   data < min_lim;     % logical is convenient, no need for loops
min_sums        =   sum(mini); % 
...
for n = 1:rows-1
    for m = 1:cols   
        ...
        if roc(n+1,m) > roc_lim                     % compare to the max rate of change
...
if cols == 1     % single column data
    for n = 1:numflags
        ...
        pct_badQA(n,1)  =   100*(numbad/numgood);
    end
...
elseif cols > 1
    for m = 1:cols
        for n = 1:numflags
            ...
            pct_badQA(n,m)  =   100*(numbad/numgood);
        end
    end
end
...
flag_types = {'1 = Exceeds Minimum Value','2 = Exceeds Maximum Value', ...
