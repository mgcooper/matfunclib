function [lat2,lon2,data2] = reducegeodata(lat,lon,data)
   ...
   %  geographic data. lat and lon are 1-d vectors of equal size. data is an
   %  array with one and only one dimension that matches the size of lat/lon,
   %  meaning an array of data values can be passed in where each element of
   %  either rows or columns of data represents values at each lat/lon pair.
   %
   % See also: reducem
