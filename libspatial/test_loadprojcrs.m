%TEST_LOADPROJCRS test loadprojcrs
%#ok<*NASGU> 
...
assert(isnan(loadprojcrs(NaN)));
assert(isinf(loadprojcrs(Inf)));
assert(abs(loadprojcrs(1e200) - 1e200) < 1e-10); % replace with theoretical result
...
eid = 'matfunclib:libname:loadprojcrs';

% verify failure
assertError(@() loadprojcrs('inputs that error'), eid, testdiag)

% verify success
assertSuccess(@() loadprojcrs('inputs that dont error'), eid, testdiag)
...
assert(isequal(loadprojcrs(X), expected));

expected = []; % add expected value for single type
assert(isequal(loadprojcrs(single(X)), single(expected)));

expected = []; % add expected value for logical type
assert(isequal(loadprojcrs(logical(X)), expected));

expected = []; % add expected value for int16 type
assert(isequal(loadprojcrs(int16(X)), expected)); % int16

%% Test dimension handling

% Test different dimensions
assert(isequal(loadprojcrs([2 3]), [4 6])); % 1D array
assert(isequal(loadprojcrs([2 3; 4 5]), [4 6; 8 10])); % 2D array
