...
% This example, manually choses the points and provides the neighborhood
% matrix in nInterp.
clear;clc;close all
%% Initializing Part I
disp('- Initializing')
...
nPoly=3; % We gonna fit a first order polynomial,
         % Note that
...
%% Generating nInterp: 
% instead of using nInterp as number of points to be selected; in this example
% Neighborhood matrix is provided.
% since this is a structured grid, prettu much all points surrounding the
% destination point are manually selected by their index.
disp('- Generating neighborhood matrix')
[idx1,idx2,idx3]=ndgrid(1:(nPoly+1));
idx1=idx1(:)';
idx2=idx2(:)';
idx3=idx3(:)';
nInterp=cell(numel(xc),1);
nCoef=nPoly+1;
halfnCoef=floor(nCoef/2);
for k=1:size(xc,3)
  for j=1:size(xc,2)
    for i=1:size(xc,1)
      ...
      nInterp{sub2ind(size(xc),i,j,k)}=sub2ind(size(xn),Iindex(idx1),Jindex(idx2),Kindex(idx3));
    end
  end
end
