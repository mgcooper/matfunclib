%% Structured grid
% This is a sample code showing how to use ConstructPolyInterpolant2D().
% Pay attention that no data is needed to construct the interpolator. Only
% the coordinates of the points on source and destination grid are needed.
clear;clc;close all
%% Initializing Part I
disp('- Initializing')
xMin=0;
...
nPoly=3; % We gonna fit a third order polynomial, 
nInterp=-randi( [30,50], ... % Using variable number of points, changing between 30 to 50.
                [(nx-1)*(ny-1),1] ... % size is Ndx1 where Nd is the number of points on destination grid.
                ); 
