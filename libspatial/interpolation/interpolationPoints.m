function Points = interpolationPoints(poly, xgrid, ygrid, buffer, newRes, ...
      varargin)
   ...
   if opts.makeplot
      ...
      h.subplot(1) = subplot(1, 3, 1); hold on
      h.grid(1) = scatter(xgrid, ygrid, 12, opts.plotvar(:), 'filled');
      h.poly(1) = plot(poly);
      h.ax(1) = gca;
      ...
      h.subplot(2) = subplot(1, 3, 2); hold on
      h.grid(2) = scatter(xgrid(idx), ygrid(idx), 60, 'filled');

      % Don't label more than 100 points
      if sum(idx) < 100
      ...
      h.subplot(3) = subplot(1, 3, 3); hold on
      h.grid(3) = scatter(xq, yq, 20, 'filled');
      h.poly(3) = plot(poly);
      h.ax(3) = gca;
