%TEST_DMSTODEGREES test dmsToDegrees
...
testdiag = "fail on negative seconds";
eid = 'matfunclib:dmsToDegrees:badMinutesOrSeconds';

% verify failure
assertError(@() dmsToDegrees(dms), eid, testdiag)

% verify success
assertSuccess(@() dmsToDegrees([1,2,3]), eid, testdiag)
...
% assert(isequal(dmsToDegrees(X), expected));
% 
% expected = []; % add expected value for single type
% assert(isequal(dmsToDegrees(single(X)), single(expected)));
% 
% expected = []; % add expected value for logical type
% assert(isequal(dmsToDegrees(logical(X)), expected));
% 
% expected = []; % add expected value for int16 type
% assert(isequal(dmsToDegrees(int16(X)), expected)); % int16

%% Test dimension handling

% Test different dimensions
% assert(isequal(dmsToDegrees([2 3]), [4 6])); % 1D array
% assert(isequal(dmsToDegrees([2 3; 4 5]), [4 6; 8 10])); % 2D array
...
%% Test invalid inputs
try
   dmsToDegrees('invalid');
   error('Expected an error for invalid inputs, but none was thrown');
catch ME
   % if arguments block is used:
   assert(strcmp(ME.identifier, 'matfunclib:dmsToDegrees:wrongNumberofColumns'));
   % otherwise, might be:
