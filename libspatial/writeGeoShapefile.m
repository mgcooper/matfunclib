...
function coords = preplatlon(fields,coords)
   ...
   latfields   = {'LATITUDE','Lat','latitude','lat','LAT'};
   lonfields   = {'LONGITUDE','Lon','longitude','lon','LON','long','LONG'};
   xfields     = {'X','x'};
   yfields     = {'Y','y'};
...
%          if strcmp(fields{ilat(n)},'Lat') || strcmp(fields{ilat(n)},'Latitude')
   ...
   if all(~ismember(fields(ilat),'Lat'))
      if isscalar(coords)  % should never be true
         coords   = renameStructField(coords,fields{ilat},'Lat');
      else
   ...
   if all(~ismember(fields(ilon),'Lon'))
      if isscalar(coords)  % should never be true
         coords   = renameStructField(coords,fields{ilon},'Lon');
      else
