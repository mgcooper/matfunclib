function varargout = myscatter(x, y, varargin)
   %MYSCATTER alias for scatter(x, y, 'filled')
   %
   %
   % See also:
   ...
   if numargs>0
      ...
      switch size(chkarg,2) % number of columns
         ...
         case 3
            % means 1st arg has three columns
            ...
            if (numargs-1) == 1
               ...
               elseif all(size(varargin{numargs}) == size(x))
                  ...
                  % means 2nd arg is an array that doesn't match
                  % the size of x and y, which isn't supported
                  error('unsupported input');
               end
            end
         case 1
            % means 1st arg has one column
            if isscalar(chkarg)
