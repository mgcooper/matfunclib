function [hFig,hTiles] = tileFigure(tileSpacing,Padding,numTiles,varargin)
   %TILEFIGURE Create a tiled layout figure with defaults.
   %
   % [hFig,hTiles] = tileFigure()
   % [hFig,hTiles] = tileFigure(tileSpacing)
   % [hFig,hTiles] = tileFigure(tileSpacing, Padding)
   % [hFig,hTiles] = tileFigure(tileSpacing, Padding, numTiles)
   %
   % See also:
