function h = trendplot(t,y,varargin)
   ...
   p.addRequired('t', @isdatelike);
   p.addRequired('y', @isnumeric);
   p.addParameter('units', '', @ischar);
   p.addParameter('ylabeltext', '', @ischar);
   p.addParameter('xlabeltext', '', @ischar);
   p.addParameter('titletext', '', @ischar);
   p.addParameter('legendtext', '', @ischar);
   p.addParameter('method', 'ols',@ischar);
   p.addParameter('alpha', 0.05, @isnumeric);
   p.addParameter('anomalies', true, @islogical);
   p.addParameter('quantile', nan, @isnumeric);
   p.addParameter('figpos', dpos, @isnumeric);
   p.addParameter('useax', nan, @isaxis);
   p.addParameter('showfig', true, @islogical);
   p.addParameter('errorbars', false,@islogical);
   p.addParameter('errorbounds', false,@islogical);
   p.addParameter('reference', nan, @isnumeric);
   p.addParameter('yerr', nan, @isnumeric);
...
function [ab,err,yfit,yci] = computeTrends(t,y,method,alpha,qtl)
   ...
   % note, conf int's not symmetric for quantile regression, so I use the mean
   % of the lower and upper for now
   ...
   for n = 1:ncol
      if isnan(qtl)
         switch method
            case 'ts'
               % need to eventually merge tsregr and ktaub, the latter doesn't
               % return the intercept and the former doesn't return conf levels
               % or much other detail.
            ...
            case 'ols'
               ...
               if isnan(alpha)
                  ...
                  % [B,CI] = regress(y(:,n),[ones(size(t)) t],alpha); CB(:,1) =
                  % CI(1,1)+CI(2,1)*t; CB(:,2) = CI(1,2)+CI(2,2)*t; CB =
                  % anomaly(CB);          % convert to anomalies?
...
function [h,makeleg,legidx] = updateFigure(useax,showfig,figpos,errorbounds)
   ...
   % the legend is parented by the figure, so if the figure contains subplots, i
   % can't just use findobj(gcf,'Type','Legend') to determine if the current
   % plot has a legend already, which i want because I want to add the next
   % trendplot trendline to the existing legend
   ...
   % assume only trendplots exist, each one has data + trend line, so the number
   % of trendlines is numlines/2, unless errorbounds is true
   ...
   % if only the error bars have handle visibility on, this should work. also
   % this shows that i can figure out how many have handlevis on and use that to
   % determine thislineidx. note, thislineidx is used to set the color order, so
   % repeated calls to trendplot use the same color for the line/patch/errorbar,
   % but it is also used to set the new legend text
   numlines = numel(lineobjs);
   %thislineidx = numlines+1;
   numpatches = numel(patchobjs);
   numerrorbars = numel(errorobjs);

   % if we have one timeseries plotted, there will be two lineobj's (one for the
   % timeseries, and one for the trendline), so we want thislineidx to evaluate
   % to 2, but I think if errorbar is true, it means there will be one line
   % object (the trendline)
   ...
   % % this is the original if errorbounds == true && mod(numchilds,2) == 1
   %    numlines = (numchilds-1)/2; thislineidx = numlines+1;
   % else
   %    numlines = numel(axchildren)/2; thislineidx = numlines+1;
   % end
   ...
   % % this version hides the -o lines so the legend shows - lines h.plot =
   % plot(useax,t,y,'-o','HandleVisibility','off',plotvarargs{:}); h.trend =
   % plot(useax,t,ytrend,'-','Color',h.plot.Color,'LineWidth',1);
   % formatPlotMarkers;
end
...
function h = plotTrend(h,t,y,yfit,yerr,yci,errorbars,errorbounds,varargs)
   ...
   % todo: use formaterrorbar instead of formatplotmarkers
   ...
   if errorbars == true
      ...
      h.plot = errorbar(h.ax,t,y,yerr, 'Marker', 'o', 'LineWidth', 1.5, ...
         'LineStyle', '-', 'MarkerEdgeColor', [.5 .5 .5] );
   else % trendlines
...
function h = drawLegend(h,ab,legtext,units,alpha,err,makeleg,legidx,precision)
   ...
   % only draw a legend if trend units were provided if not(isempty(trendunits))
   ...
   % ceil(abs(log10(0.003))) = 3 ceil(abs(log10(300))) = 3 +1 gets you an extra
   % digit of precision
