function [h,edges,centers,hfit] = loghist(data,varargin)
   ...
   addRequired(parser,'data',@(x)isnumeric(x));
   addParameter(parser,'normalization','pdf',@(x)ischar(x));
   addParameter(parser,'logmodel','loglog',@(x)ischar(x));
   addParameter(parser,'edges',logbinedges(data),@(x)isnumeric(x));
   addParameter(parser,'centers',logbincenters(data),@(x)isnumeric(x));
   addParameter(parser,'dist','none',@(x)ischar(x));
   addParameter(parser,'theta',0,@(x)isnumeric(x));
