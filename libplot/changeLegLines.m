function changeLegLines( l,scaleFactor )
%CHANGELEGLINES Change the size of the legend lines. Currently only
%supports lines and patches and only works if legend is located in default
%vertical stacked position
...
linesInPlot     =   findobj(legchilun,'type','line');
patchInPlot     =   findobj(legchilun,'type','patch');
textInPlot      =   findobj(legchilun,'type','text');
...
numpatches  =   length(patchXdata)/4;
