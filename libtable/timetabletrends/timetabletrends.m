...
% regression goes on 1:numyears rather than year1:1:yearFinal, which creates a
% problem if later you want to use the ab values to predict something on the
% original times. BUT, only the intercept should be affected, so I added the
% regressor to the table. UPDATE UPDATE: regressing on years might be best. For
% short-time data, force the user to regress on days, hours, minutes, or
% seconds, no months
