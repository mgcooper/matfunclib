%TEST_REORDERVARS test reordervars
%#ok<*NASGU> 
...
assert(isnan(reordervars(NaN)));
assert(isinf(reordervars(Inf)));
assert(abs(reordervars(1e200) - 1e200) < 1e-10); % replace with theoretical result
...
eid = 'matfunclib:libname:reordervars';

% verify failure
assertError(@() reordervars('inputs that error'), eid, testdiag)

% verify success
assertSuccess(@() reordervars('inputs that dont error'), eid, testdiag)
...
assert(isequal(reordervars(X), expected));

expected = []; % add expected value for single type
assert(isequal(reordervars(single(X)), single(expected)));

expected = []; % add expected value for logical type
assert(isequal(reordervars(logical(X)), expected));

expected = []; % add expected value for int16 type
assert(isequal(reordervars(int16(X)), expected)); % int16

%% Test dimension handling

% Test different dimensions
assert(isequal(reordervars([2 3]), [4 6])); % 1D array
assert(isequal(reordervars([2 3; 4 5]), [4 6; 8 10])); % 2D array
