classdef testTimetablereduce < matlab.unittest.TestCase
   %TESTTIMETABLEREDUCE Test timetablereduce and renametimetabletimevar.
   %
   % timetablereduce calls renametimetabletimevar before it reads the row
   % times, so inputs with a time dimension named 'Date' test the rename
   % step. Expected statistics are hand-computed from the data: the mean and
   % sample standard deviation of the non-NaN values, SE = sigma/sqrt(N), and
   % CIL and CIH = mu -/+ PM. Every sample has N < 5, so stderr sets
   % PM = sf*SE, with sf = 1 for alpha = 0.32 and sf = 2 for alpha = 0.05.
   %
   % Three timetablereduce branches have no test because they error for the
   % inputs that reach them. The property-copy block errors for an input
   % with VariableUnits or VariableContinuity. A dim other than 1 or 2
   % leaves NewData unassigned. One column with keeptime = false errors in
   % setnan when the column has two or more valid values. A test here would
   % assert the defect.
   ...
   methods (TestClassSetup)
      function addProjectToPath(testCase)
         ...
         % Put matfunclib on the path so timetablereduce and its helpers
         % resolve from the repo root without a manual addpath.
      ...
      function makedata(testCase)
         % Store the shared data once. The data use a time dimension named
         % 'Date' so every call renames it.
   ...
   methods (Test)
      function testRenameHelperRenamesTimeDimension(testCase)
         ...
         returned = renametimetabletimevar(testCase.TT);
         ...
         table_returned = renametimetabletimevar(T);
         testCase.verifyEqual(table_returned.Properties.DimensionNames, ...
            dims_expected)
      end

      function testRenameHelperKeepsTimeDimension(testCase)
         ...
         returned = renametimetabletimevar(TTtime);
         testCase.verifyEqual(returned, expected)
      end

      function testReduceRows(testCase, rowoptions)
         ...
         returned = testCase.verifyWarningFree( ...
            @() timetablereduce(testCase.TT, args{:}));
      ...
      function testReduceColumns(testCase)
         ...
         % the transposed array taller than wide, so stderr does not warn.
      ...
      function testSingleColumnKeepTime(testCase)
         ...
         returned = timetablereduce(T1, 'keeptime', true);
      ...
      function testInputValidation(testCase)
         ...
         testCase.verifyError(@() timetablereduce([1, 2; 3, 4]), errid)
         testCase.verifyError(@() timetablereduce(testCase.TT, ...
            'keeptime', 1), errid)
      end
   end
end
...
function verifystats(testCase, NewData, mu_expected, sigma_expected, ...
   ...
   % tolerance covers floating-point rounding only.
   tol = 1e-12;
