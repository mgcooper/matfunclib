%TEST_TOTIMETABLE test totimetable
%#ok<*NASGU> 
...
assert(isnan(totimetable(NaN)));
assert(isinf(totimetable(Inf)));
assert(abs(totimetable(1e200) - 1e200) < 1e-10); % replace with theoretical result
...
eid = 'matfunclib:libname:totimetable';

% verify failure
assertError(@() totimetable('inputs that error'), eid, testdiag)

% verify success
assertSuccess(@() totimetable('inputs that dont error'), eid, testdiag)
...
assert(isequal(totimetable(X), expected));

expected = []; % add expected value for single type
assert(isequal(totimetable(single(X)), single(expected)));

expected = []; % add expected value for logical type
assert(isequal(totimetable(logical(X)), expected));

expected = []; % add expected value for int16 type
assert(isequal(totimetable(int16(X)), expected)); % int16

%% Test dimension handling

% Test different dimensions
assert(isequal(totimetable([2 3]), [4 6])); % 1D array
assert(isequal(totimetable([2 3; 4 5]), [4 6; 8 10])); % 2D array
