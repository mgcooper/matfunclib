function T = tablecleaner(T,varargin)
   ...
   %
   % parser.addRequired(    'T',                       validtime   );
   % parser.addParameter(   'regularize',  false,      validreg    );
   % parser.addParameter(   'newtime',     NaT,        validnewt   );
   % parser.addParameter(   'method',      'linear',   validmethod );
   % parser.addParameter(   'whichrow',    'first',    validrow    );

   parser.addRequired('T', @istabular);
   parser.addParameter('regularize',false, @islogical);
   parser.addParameter('newtime',  NaT, @isdatelike);
   parser.addParameter('method', 'linear', @ischar);
   parser.addParameter('whichrow', 'first', @ischar);

   % use first row as default when replacing duplicates

   parser.parse(T, varargin{:});
   regularize = parser.Results.regularize;
   newtime = parser.Results.newtime;
   method = parser.Results.method;
   whichrow = parser.Results.whichrow;
