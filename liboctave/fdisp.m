...
% cell array are not supported. By default, it sets a limit to the size
% of arrays it will print. For example, if an array is larger than 1e6 elements,
% which roughly equals 8 MB for double arrays, 4 MB for single arrays, and 1 MB for chars,
% the function will print a warning and not print the array in full.
% You can override this by passing true as the third argument or specify a different
% size limit by passing a numeric value as the third argument.
%
%
% INPUT:
...
if nargin == 3
   if isscalar(varargin{1})
      ...
      elseif islogical(varargin{1})
         ...
         error('fdisp:InvalidInput', 'Third argument should be numeric or logical');
      end
   else
...
if isnumeric(x) || islogical(x)
   if numel(x) <= sizelimit || ignore_sizelimit
      ...
      warning('Array size exceeds the limit and will not be printed in full. Pass true as the third argument to override.');
   ...
   elseif all(cellfun(@isnumeric, x) | cellfun(@islogical, x))
      for i = 1:numel(x)
         if numel(x{i}) <= sizelimit || ignore_sizelimit
            ...
            warning('Array size in cell element %d exceeds the limit and will not be printed in full. Pass true as the third argument to override.', i);
            fprintf(fid, '[%s %s]\n', mat2str(size(x{i})), class(x{i}));
         end
      end
   else
      error('fdisp:InvalidInput', 'Mixed data types within cell array not supported');
   end
else
