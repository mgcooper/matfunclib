...
p.addParameter('plotfig',false,@(x)islogical(x));
...
% note: water year + snow correction totally throws off the GL timing


for n = 1:nyears
   
   [imin(n),GL(n)]  =  findmin(GraceMY(validmonthsmin,n),1,'last');
   [imax(n),GH(n)]  =  findmax(GraceMY(validmonthsmax,n),1,'last');
   
   % correct for validmonths offset
   imin(n) = imin(n) + validmonthsmin(1) - 1;
   imax(n) = imax(n) + validmonthsmax(1) - 1;

   if plotcheck == true
      if n==1; figure; end
   
      monthplot(GraceMY(:,n),'wateryear',wateryear); hold on;
      if ~isnan(imin(n)) 
