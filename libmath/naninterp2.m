function V = naninterp2(X,Y,V,method,extrap)
   ...
   %   underlying function V=F(X) at the query points Xq, Yq which are the
   %   values of X, Y where V is nan.
   %
   ...
   % Update: I think this needs to be rewritten. I think extrap has to be true
   % for the nans to be filled. But I have not tested this when the missing
   % value is in the middle of the array, so until then, keep the extrap = NaN.
   ...
   if nargin == 1
      if isvector(V)
         ...
         % the correct indexing when using X=1:n, Y=1:m, instead if V is a
         % vector, must use the syntax interp2(V, Xq, Yq, method);
   ...
   % values, or on scattered data like scatteredInterpolant, but the results
   % seem to differ esp. the errors that occur e.g. when nan values lie near the
   % boundary.
